...
public class ProgressiveInputFile : InputFile, IAsyncDisposable
{
	
	public static InputFileCoreSettings Defaults { get; }

	static ProgressiveInputFile()
	{
	...
	protected long MaxFileSizeEffective => this.MaxFileSize ?? this.GetSettings()?.MaxFileSize ?? GetDefaults().MaxFileSize ?? throw new InvalidOperationException(nameof(MaxFileSize) + " default for " + nameof(ProgressiveInputFile) + " has to be set.");

	/// <summary>
	/// Maximum number of concurrent uploads. Default is <c>6</c>.
	/// </summary>
	[Parameter] public int? MaxParallelUploads { get; set; }
	protected int MaxParallelUploadsEffective => this.MaxParallelUploads ?? this.GetSettings()?.MaxParallelUploads ?? this.GetDefaults().MaxParallelUploads ?? throw new InvalidOperationException(nameof(MaxParallelUploads) + " default for " + nameof(ProgressiveInputFile) + " has to be set.");
	...
	public async Task StartUploadAsync(string accessToken = null)
	{
		...
		await _jsModule.InvokeVoidAsync("upload", Id, dotnetObjectReference, UploadUrl, accessToken, MaxFileSizeEffective == long.MaxValue ? null : MaxFileSizeEffective, MaxParallelUploadsEffective);
	}
    ...
    {
        _jsModule ??= await JsRuntime.ImportHavitBlazorWebModuleAsync(nameof(ProgressiveInputFile));
    }
